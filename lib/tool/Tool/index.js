...
class Tool{
    ...
    init(){
        ...
        addCss('.art-toolbar{color: #666; font-weight:600; font-size:13.5px; border-radius: 4px}');
    ...
    setFolatToolbar(sel='hidden'){
        ...
        this.floatToolbar.style.top = (event.pageY ).toString() + 'px';
        this.floatToolbar.style.left = (event.pageX).toString() + 'px';
    ...
    addDefaultTool(){
        ...
        this.addTool('保存本地', 
        ()=>{localStorage.md = window.artText.interpreter.getMd();message('md保存成功', 'success'); console.log('md保存成功')});

        this.addTool('本地载入', 
        ()=>{window.artText.interpreter.setMd(localStorage.md); message('md载入成功', 'success');console.log('md载入成功')});
    }
