...
class Tool{
    ...
    init(){
        ...
        this.floatToolbar.style.backgroundColor = this.artText.config.theme.backgroundColor;
        addCss('.art-floatToolbar-span{padding: 8px 10px;cursor: pointer;}.art-floatToolbar-span:hover{color:' + this.artText.config.theme.color + '}');
    ...
    setFloatToolbar(sel='hidden'){
        ...
        console.log(event);
        console.log(this.artText.event.clickInfo);
        let pageXLenght  = event.pageX  - (this.artText.event.clickInfo.pageX );
        let pageYLenght  = event.pageY  - (this.artText.event.clickInfo.pageY );
        console.log(pageXLenght, pageYLenght)
        if(pageYLenght > 0){
            console.log('123')
            let event = this.artText.event.clickInfo;
            this.floatToolbar.style.top = (event.pageY - event.offsetY - 20).toString() + 'px';
            this.floatToolbar.style.left = (event.pageX + pageXLenght/2).toString() + 'px';
        }else{
            console.log('123456')
            this.floatToolbar.style.top = (event.pageY - event.offsetY - 20).toString() + 'px';
            this.floatToolbar.style.left = (event.pageX + pageXLenght/2).toString() + 'px';
