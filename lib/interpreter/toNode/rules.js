...
const bold = {"left": "**", "right": "**", "re": /\*{2}.+\*{2}/g, "fun": blodFun}
const italic = {"left": "*", "right": "*", "re": /\*.+\*/g, "fun":italicFun}
...
const hr = {"re": /^\*{3,}/, "fun": hrFun}
export const alineRule = [h1, h2, h3, h4, h5, h6, hr]
...
const blockquote = {"start": ">", "startRe": /^>\s/, "endRe": /^(?!>\s)/, "fun":blockquoteFun}
const table = {reNum: 3, startRe: /^\|(:?-{3,}\|)*/, secondRe: /^\|.*\|/, endRe: /^(?!\|.*\|)/}
export const morelineRule = []
function blockquoteFun(texts){
  return [new Node("blockquote", {}, [])]
}
function ulFun(texts){
  return [new Node("ul", {}, [])]
}
function olFun(texts){
  return [new Node("ol", {}, [])]
}
function tableFun(texts){
