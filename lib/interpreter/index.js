...
import {loadScript, loadCss} from "../tool"
...
class Interpreter{
    ...
    createRootNode(){
        let br = new Node("br")
        let p = new Node("p", {}, br)
        let root = new Node('div', {"__root__": true, contenteditable:"true", 'class': 'art-editor'}, p);
    ...
    init(model){
        ...
        let childNodes = textToNode(window.artText.config.md);
        if(childNodes){
