import { VObject, VText } from '..';
import ArtRender from '../../../';
import { imgTool, initTableTool, initCodeTool, initTocTool } from '../../../tool'

export default class VNode extends VObject {
    attr: {};
    childNodes: (VNode | VText)[];
    dom: HTMLElement;
    parentNode: VNode;

    constructor(nodeName: string, attr: {} = {}, childNodes: any[] | VText | VNode = null) {
        super(nodeName);
        ...
        for (let key in this.attr) {
            if (key === "__dom__") {
                ...
                if (ArtRender.plugins.katex) {
                    this.dom.innerHTML = ArtRender.plugins.katex.renderToString(this.attr["art-math"], { throwOnError: false });
    ...
    /**添加孩子节点 */
    public appendChild(vnode: VNode | VText): boolean {
    ...
    /**替换孩子节点 */
    public replaceChild(newNode: VNode | VText, oldNode: VNode | VText): boolean {
    ...
    /**替换所有孩子节点 */
    public replaceAllChild(newNodes: (VNode | VText)[]): boolean {
        if (newNodes) {
            this.childNodes = [];
    ...
    /**插入参考孩子节点之前 */
    public insertBefore(newChild: VNode | VText, refChild: VNode | VText): boolean {
    ...
    /**插入参考孩子节点之后 */
    public insertAfter(newChild: VNode | VText, refChild: VNode | VText): boolean {
        let index = this.childNodes.indexOf(refChild);
        if (index != -1) {
            newChild.parentNode = this;
    ...
    /**删除孩子节点 */
    public removeChild(oldChild: VText | VNode): boolean {
    ...
    public getMd(model: string = 'editor'): string {
        ...
        if (/art-toc(\s|$)/.test(this.attr['class'])) {
            ...
            md += (<VText>this.childNodes[0]).text;
